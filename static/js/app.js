...
(() => {
  ...
  // WebRTC (stubs to keep UI smooth; works if backend supported)
  const createPC = () => {
  ...
  const startCall = async () => { if(!state.peer) return showToast("اختر محادثة أولاً",{center:true}); await ensureLocal(); state.pc = createPC(); state.localStream.getTracks().forEach(t=>t.addEventListener && state.pc.addTrack(t,state.localStream)); await state.pc.setLocalDescription(await state.pc.createOffer()); socket.emit("webrtc-offer", { peer: state.peer, sdp: state.pc.localDescription }); showToast("جاري الاتصال...", {center:true,duration:1400}); };
  const acceptIncoming = async (from, sdp) => { await ensureLocal(); state.peer = from; renderChatList(); state.pc=createPC(); state.localStream.getTracks().forEach(t=>state.pc.addTrack(t,state.localStream)); await state.pc.setRemoteDescription(sdp); await state.pc.setLocalDescription(await state.pc.createAnswer()); socket.emit("webrtc-answer", { peer: from, sdp: state.pc.localDescription }); hideIncoming(); showToast("تم الاتصال ✔",{center:true,duration:1200}); };
  const endCall = () => { if(state.pc){ try{state.pc.close();}catch{} state.pc=null;} detachRemote(); showToast("تم إنهاء المكالمة",{center:true,duration:1400}); };
  ...
  socket.on("webrtc-offer", ({ from, sdp }) => { incomingFrom.textContent=`من: ${from}`; incomingModal.classList.add("show"); acceptCall.onclick=()=>acceptIncoming(from,sdp); declineCall.onclick=()=>{ incomingModal.classList.remove("show"); socket.emit("message",{ peer:from, text:"المكالمة مرفوضة الآن" }); showToast("تم رفض المكالمة",{center:true}); }; });
  socket.on("webrtc-answer", async ({ from, sdp }) => { if(!state.pc) return; await state.pc.setRemoteDescription(sdp); showToast("تم الاتصال ✔",{center:true}); });
  ...
  // Exit current chat
  const exitToList = () => { clearMessagesUI(); sidebar.classList.add("open"); overlay.classList.add("show"); showToast("تم الخروج من المحادثة",{center:true}); };
  exitChatBtn.onclick = () => {
